...
async def run_query(query: str, conversation_history: List[Dict[str, str]], clients: Dict[str, Any], grounding=False, language="Vietnamese"):
    ...
    Goal: Generate anevidence-based response to the user's diabetes-related query using both the provided knowledge graph information. If the query is a question, provide answer based on the knowledge graph information. 
    <user query>
    {query}
    </user query>
    ...
    * detailed: Comprehensive explanation with relevant details, comparing different sources and explaining nuances
    ...
    response = await clients["gemini_client"].generate(prompt=final_prompt)
